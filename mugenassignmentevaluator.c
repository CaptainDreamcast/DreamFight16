...
static AssignmentReturnValue evaluateAssignmentInternal(MugenAssignment* tAssignment, Player* tPlayer);
...
static AssignmentReturnValue evaluateOrAssignment(MugenAssignment* tAssignment, Player* tPlayer) {
...
static AssignmentReturnValue evaluateAndAssignment(MugenAssignment* tAssignment, Player* tPlayer) {
...
static AssignmentReturnValue evaluateBitwiseOrAssignment(MugenAssignment* tAssignment, Player* tPlayer) {
...
static AssignmentReturnValue evaluateBitwiseAndAssignment(MugenAssignment* tAssignment, Player* tPlayer) {
...
static AssignmentReturnValue evaluateCommandAssignment(AssignmentReturnValue tCommand, Player* tPlayer) {
...
static AssignmentReturnValue evaluateStateTypeAssignment(AssignmentReturnValue tCommand, Player* tPlayer) {
	MugenStateType playerState = getPlayerStateType(tPlayer);
...
static AssignmentReturnValue evaluateMoveTypeAssignment(AssignmentReturnValue tCommand, Player* tPlayer) {
	MugenStateMoveType playerMoveType = getPlayerStateMoveType(tPlayer);
...
static AssignmentReturnValue evaluateAnimElemVectorAssignment(AssignmentReturnValue tCommand, Player* tPlayer) {
...
static AssignmentReturnValue evaluateAnimElemNumberAssignment(AssignmentReturnValue tCommand, Player* tPlayer) {
...
static AssignmentReturnValue evaluateAnimElemAssignment(AssignmentReturnValue tCommand, Player* tPlayer) {
...
static Player* getPlayerFromFirstVectorPartOrNullIfNonexistant(AssignmentReturnValue a, Player* tPlayer) {
	char firstWord[100];
	int id;
	...
	if (items == 1 && !strcmp("p1", firstWord)) {
		...
		Player* ret = getPlayerHelperOrNullIfNonexistant(tPlayer, id);
		assert(ret);
		return ret;
	}
	else if (items == 2 && !strcmp("playerid", firstWord)) {
		Player* ret = getPlayerByIDOrNullIfNonexistant(tPlayer, id);
		assert(ret);
		return ret;
	}
	else {
...
static int isPlayerAccessVectorAssignment(AssignmentReturnValue a, Player* tPlayer) {
	return getPlayerFromFirstVectorPartOrNullIfNonexistant(a, tPlayer) != NULL;
}

static AssignmentReturnValue evaluateTeamModeAssignment(AssignmentReturnValue tCommand, Player* tPlayer) {
	(void)tPlayer;
...
static AssignmentReturnValue evaluateSetVariableAssignment(MugenAssignment* tAssignment, Player* tPlayer) {
	MugenDependOnTwoAssignment* varSetAssignment = tAssignment->mData;

	assert(varSetAssignment->a->mType == MUGEN_ASSIGNMENT_TYPE_ARRAY);
	MugenDependOnTwoAssignment* varArrayAccess = varSetAssignment->a->mData;
	AssignmentReturnValue a = evaluateAssignmentInternal(varArrayAccess->a, tPlayer);
	int index = evaluateAssignmentAndReturnAsInteger(varArrayAccess->b, tPlayer);

	if (!strcmp("var", a.mValue) || !strcmp("Var", a.mValue)) {
		int value = evaluateAssignmentAndReturnAsInteger(varSetAssignment->b, tPlayer);
		...
		double value = evaluateAssignmentAndReturnAsFloat(varSetAssignment->b, tPlayer);
...
static AssignmentReturnValue evaluateComparisonAssignment(MugenAssignment* tAssignment, Player* tPlayer) {
	...
	if (comparisonAssignment->a->mType == MUGEN_ASSIGNMENT_TYPE_VECTOR) {
		...
		if (isPlayerAccessVectorAssignment(vecA, tPlayer)) {
			Player* target = getPlayerFromFirstVectorPartOrNullIfNonexistant(vecA, tPlayer);
			assert(target);
...
static AssignmentReturnValue evaluateInequalityAssignment(MugenAssignment* tAssignment, Player* tPlayer) {
...
static AssignmentReturnValue evaluateGreaterAssignment(MugenAssignment* tAssignment, Player* tPlayer) {
...
static AssignmentReturnValue evaluateGreaterOrEqualAssignment(MugenAssignment* tAssignment, Player* tPlayer) {
...
static AssignmentReturnValue evaluateLessAssignment(MugenAssignment* tAssignment, Player* tPlayer) {
...
static AssignmentReturnValue evaluateLessOrEqualAssignment(MugenAssignment* tAssignment, Player* tPlayer) {
...
static AssignmentReturnValue evaluateModuloAssignment(MugenAssignment* tAssignment, Player* tPlayer) {
...
static AssignmentReturnValue evaluateExponentiationAssignment(MugenAssignment* tAssignment, Player* tPlayer) {
...
static AssignmentReturnValue evaluateMultiplicationAssignment(MugenAssignment* tAssignment, Player* tPlayer) {
...
static AssignmentReturnValue evaluateDivisionAssignment(MugenAssignment* tAssignment, Player* tPlayer) {
...
static AssignmentReturnValue evaluateAdditionAssignment(MugenAssignment* tAssignment, Player* tPlayer) {
...
static AssignmentReturnValue evaluateSubtractionAssignment(MugenAssignment* tAssignment, Player* tPlayer) {
...
static AssignmentReturnValue evaluateOperatorArgumentAssignment(MugenAssignment* tAssignment, Player* tPlayer) {
...
static AssignmentReturnValue evaluatePlayerVectorAssignment(AssignmentReturnValue tFirstValue, MugenDependOnTwoAssignment* tVectorAssignment, Player* tPlayer) {
	Player* target = getPlayerFromFirstVectorPartOrNullIfNonexistant(tFirstValue, tPlayer);
	assert(target != NULL);
...
static AssignmentReturnValue evaluateVectorAssignment(MugenAssignment* tAssignment, Player* tPlayer) {
...
static AssignmentReturnValue evaluateRangeAssignment(MugenAssignment* tAssignment, Player* tPlayer) {
...
static AssignmentReturnValue evaluateVariableAssignment(MugenAssignment* tAssignment, Player* tPlayer) {
	...
	if (!strcmp("stateno", testString)) {
		...
		return makeFloatAssignmentReturn(getCameraPositionX(getPlayerCoordinateP(tPlayer)));
	}
		...
		return makeNumberAssignmentReturn(getGameTime());
	}
		...
		return makeNumberAssignmentReturn(getRoundStateNumber());
	}
		...
		return makeBooleanAssignmentReturn(isMatchOver(tPlayer));
	}
	else if (!strcmp("roundno", testString)) {
		return makeNumberAssignmentReturn(getRoundNumber());
	}
	else if (!strcmp("matchno", testString)) {
		return makeNumberAssignmentReturn(getMatchNumber());
	}
		...
		return makeNumberAssignmentReturn(getGameWidth(getPlayerCoordinateP(tPlayer)));
	}
	...
	else if (doAssignmentStringsBeginsWithPattern("projhit", testString)) { // TODO: fix
		return makeBooleanAssignmentReturn(0); // TODO: implement
	}
	else if (doAssignmentStringsBeginsWithPattern("projcontact", testString)) { // TODO: fix
		return makeBooleanAssignmentReturn(0); // TODO: implement
	}
	else {
...
static AssignmentReturnValue evaluateVarArrayAssignment(AssignmentReturnValue tIndex, Player* tPlayer) {
...
static AssignmentReturnValue evaluateSysVarArrayAssignment(AssignmentReturnValue tIndex, Player* tPlayer) {
	int id = evaluateAssignmentReturnAsNumber(tIndex);
...
static AssignmentReturnValue evaluateFVarArrayAssignment(AssignmentReturnValue tIndex, Player* tPlayer) {
	int id = evaluateAssignmentReturnAsNumber(tIndex);
...
static AssignmentReturnValue evaluateSysFVarArrayAssignment(AssignmentReturnValue tIndex, Player* tPlayer) {
	int id = evaluateAssignmentReturnAsNumber(tIndex);
...
static AssignmentReturnValue evaluateConstArrayAssignment(AssignmentReturnValue tIndex, Player* tPlayer) {
...
static AssignmentReturnValue evaluateGetHitVarArrayAssignment(AssignmentReturnValue tIndex, Player* tPlayer) {
	char* var = tIndex.mValue;
...
static AssignmentReturnValue evaluateAnimationElementTimeArrayAssignment(AssignmentReturnValue tIndex, Player* tPlayer) {
	int id = evaluateAssignmentReturnAsNumber(tIndex);
	int ret = getPlayerTimeFromAnimationElement(tPlayer, id);
	return makeNumberAssignmentReturn(ret);
}

static AssignmentReturnValue evaluateAnimationElementNumberArrayAssignment(AssignmentReturnValue tIndex, Player* tPlayer) {
...
static AssignmentReturnValue evaluateCondArrayAssignment(MugenAssignment* tCondVector, Player* tPlayer) {
	...
	AssignmentReturnValue ret;
	int isTrue = evaluateAssignment(firstV->a, tPlayer);
	if (isTrue) {
...
static AssignmentReturnValue evaluateSelfAnimationExistArrayAssignment(AssignmentReturnValue tIndex, Player* tPlayer) {
	int id = evaluateAssignmentReturnAsNumber(tIndex);
...
static AssignmentReturnValue evaluateConstCoordinatesArrayAssignment(AssignmentReturnValue tIndex, int p) {
	...
	double ret = parseCoordinatesToLocalCoordinateSystem(coords, p);
...
static AssignmentReturnValue evaluateNumTargetArrayAssignment(AssignmentReturnValue tIndex, Player* tPlayer) {
	int id = evaluateAssignmentReturnAsNumber(tIndex);
	int ret = getPlayerTargetAmountWithID(tPlayer, id);
	return makeNumberAssignmentReturn(ret);
}

static AssignmentReturnValue evaluateNumHelperArrayAssignment(AssignmentReturnValue tIndex, Player* tPlayer) {
	int id = evaluateAssignmentReturnAsNumber(tIndex);
	int ret = getPlayerHelperAmountWithID(tPlayer, id);
	return makeNumberAssignmentReturn(ret);
}

static AssignmentReturnValue evaluateNumExplodArrayAssignment(AssignmentReturnValue tIndex, Player* tPlayer) {
	int id = evaluateAssignmentReturnAsNumber(tIndex);
	int ret = getExplodAmountWithID(tPlayer, id);
	return makeNumberAssignmentReturn(ret);
}

static AssignmentReturnValue evaluateIsHelperArrayAssignment(AssignmentReturnValue tIndex, Player* tPlayer) {
	int id = evaluateAssignmentReturnAsNumber(tIndex);
...
static AssignmentReturnValue evaluatePlayerIDExistArrayAssignment(AssignmentReturnValue tIndex, Player* tPlayer) {
	int id = evaluateAssignmentReturnAsNumber(tIndex);
...
static AssignmentReturnValue evaluateUnaryMinusAssignment(MugenAssignment* tAssignment, Player* tPlayer) {
...
static AssignmentReturnValue evaluateNegationAssignment(MugenAssignment* tAssignment, Player* tPlayer) {
