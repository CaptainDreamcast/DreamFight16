...
static void handleInputStepWithMultipleSubsteps(Vector* tVector, char* tInputStep, int tDoesNotAllowOtherInputBetween) {
	MugenCommandInputStep* multipleStep = allocMemory(sizeof(MugenCommandInputStep));
	...
	MugenCommandInputStepMultipleTargetData* data = allocMemory(sizeof(MugenCommandInputStepMultipleTargetData));
...
static MugenCommandInputStepTarget extractTargetFromInputStep(char* tInputStep) {
	...
	MugenCommandInputStepTarget ret;
...
static void handleHoldingInputStep(Vector* tVector, char* tInputStep, int tDoesNotAllowOtherInputBetween) {
	MugenCommandInputStep* e = allocMemory(sizeof(MugenCommandInputStep));
...
static void handleReleaseStep(Vector* tVector, char* tInputStep, int tDoesNotAllowOtherInputBetween) {
	MugenCommandInputStepReleaseData* data = allocMemory(sizeof(MugenCommandInputStepReleaseData));
...
static void addCallerToExistingCommand(MugenCommand* tCommand, CommandCaller* tCaller) {
	MugenCommandInput* input = allocMemory(sizeof(MugenCommandInput));
	*input = tCaller->mInput;

	vector_push_back_owned(&tCommand->mInputs, input);
}

static void addEmptyCommandToCommands(MugenCommands* tCommands, char* tName) {
	MugenCommand* e = allocMemory(sizeof(MugenCommand));
...
static void addCallerToCommands(MugenCommands* tCommands, CommandCaller* tCaller) {
	...
	MugenCommand* command = string_map_get(&tCommands->mCommands, tCaller->mName);
	addCallerToExistingCommand(command, tCaller);

}

static void handleCommand(MugenCommands* tCommands, MugenDefScriptGroup* tGroup) {
...
static void loadMugenCommandsFromDefScript(MugenCommands* tCommands, MugenDefScript* tScript) {
...
static MugenCommands makeEmptyMugenCommands() {
	MugenCommands ret;
...
MugenCommands loadMugenCommandFile(char * tPath)
{
	...
	MugenCommands ret = makeEmptyMugenCommands();
