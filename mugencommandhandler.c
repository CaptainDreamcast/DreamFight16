...
static int handleSingleCommandInputStepAndReturnIfActive(MugenCommandInputStep* tStep, int* oIsStepOver, Player* tPlayer, int tIsFacingRight);
...
static void handleSingleMultipleCommandInputStep(void* tCaller, void* tData) {
	...
	MugenCommandInputStep* step = tData;
...
static int handleMultipleCommandInputStep(MugenCommandInputStep* tStep, int* oIsStepOver, Player* tPlayer, int tIsFacingRight) {
	MugenCommandInputStepMultipleTargetData* data = tStep->mData;
...
static int isButtonCommandActive(MugenCommandInputStepTarget tTarget, uint32_t tMask, int isFacingRight) {
	int directionMask = 0;
...
static int isTargetHeld(MugenCommandInputStepTarget tTarget, Player* tPlayer, int tIsFacingRight) {
	return isButtonCommandActive(tTarget, gData.mHeldMask[tPlayer->mControllerID], tIsFacingRight);
}

static int isTargetPressed(MugenCommandInputStepTarget tTarget, Player* tPlayer, int tIsFacingRight) {
...
static int isTargetReleased(MugenCommandInputStepTarget tTarget, Player* tPlayer, int tIsFacingRight) {
	return !isButtonCommandActive(tTarget, gData.mHeldMask[tPlayer->mControllerID], tIsFacingRight) && isButtonCommandActive(tTarget, gData.mPreviousHeldMask[tPlayer->mControllerID], tIsFacingRight);
}

static int handleHoldingCommandInputStep(MugenCommandInputStep* tStep, int* oIsStepOver, Player* tPlayer, int tIsFacingRight) {
...
static int handlePressingCommandInputStep(MugenCommandInputStep* tStep, int* oIsStepOver, Player* tPlayer, int tIsFacingRight) {
...
static int handleReleasingCommandInputStep(MugenCommandInputStep* tStep, int* oIsStepOver, Player* tPlayer, int tIsFacingRight) {
...
static int isSameStepAsBefore(ActiveMugenCommand* tCommand) {
	...
	MugenCommandInputStep* mPreviousStep = vector_get(&tCommand->mInput->mInputSteps, tCommand->mStep - 1);
	MugenCommandInputStep* mStep = vector_get(&tCommand->mInput->mInputSteps, tCommand->mStep);
...
static int updateSingleActiveMugenCommand(void* tCaller, void* tData) {
	...
	int isRunning = 1;
	while (isRunning) {
		MugenCommandInputStep* step = vector_get(&command->mInput->mInputSteps, command->mStep);
...
static void addNewActiveMugenCommand(MugenCommandInput* tInput, RegisteredMugenCommand* tRegisteredCommand, char* tName, int mIsStepOver) {
...
static void updateSingleStaticMugenCommandInput(void* tCaller, void* tData) {
	MugenCommandInput* input = tData;
	StaticMugenCommandInputCaller* caller = tCaller;

	MugenCommandInputStep* firstStep = vector_get(&input->mInputSteps, 0);
...
ActorBlueprint MugenCommandHandler = {
